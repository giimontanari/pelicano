...
import { actChangeGamesProps, actGetMyGames } from "../actions/gameAction";
...
/**
 * Este componente muestra una UI que renderiza los juegos creados para un usuario 
 * autenticado en la plataforma.
 */
class MyGamesView extends Component {
  constructor(props) {
    super(props);
    this.state = {};
  ...
  async componentDidMount() {
    ...
    await this.props.actGetMyGames(uid);
  }

  setSelectGame(game) {
    this.props.actChangeGamesProps(
      { gameSelect: game, currentLevel: this.props.myGames.indexOf(game) },
          ...
          {this.props.myGames.length > 0 ? (
            <div className={classes.container}>
              <GridList cellHeight={305} className={classes.gridList} cols={2}>
                {this.props.myGames.map((game, index) => {
                  return (
...
export default withRouter(
  connect(
    mapStateToProps,
    mapDispatchToProps
  )(withStyles(useStyles)(MyGamesView))
);
